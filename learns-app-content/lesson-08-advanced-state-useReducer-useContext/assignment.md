...
> - "I built a debounced search that waits 300ms before sending an API request. Here's my explanation of how the `useDebounce` hook delays the value update and why the `useEffect` that calls the API depends on the debounced value instead of the raw input: [my explanation]. Can you check my understanding and ask me one follow-up question?"
